...
Ask all five questions in **one message**. Take free prose; do not force choices.
...
- Write the answers **verbatim** into `campaigns/<slug>/brief.md` under the six
...
## Step 8 — Evaluate, one batch at a time
...
- **API keys come from environment variables only** (`APIFY_TOKEN`,
  `FIRECRAWL_API_KEY`). Never ask for a key in chat and never write one to a file.
- **Re-running a campaign** is cheap: the crawl cache persists per domain, so
  only the evaluation repeats.
